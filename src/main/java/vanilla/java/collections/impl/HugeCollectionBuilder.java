...
import vanilla.java.collections.model.VanillaTypeModel;
...
public abstract class HugeCollectionBuilder<T> {
    ...
    protected HugeCollectionBuilder(int typeParameter) {
        ...
        typeModel = new VanillaTypeModel<T>(type);
    ...
    public HugeCollectionBuilder<T> partitionSize(int partitionSize) {
    ...
    public long capacity() {
        return Math.max(Math.max(partitionSize, capacity), MIN_ALLOCATION_SIZE);
    }
    ...
    protected void normaliseArgs() {
        ...
        if (partitionSize < MIN_ALLOCATION_SIZE) {
            partitionSize = MIN_ALLOCATION_SIZE;
            while (128 * partitionSize < capacity && partitionSize < 64 * 1024 * 1024)
                partitionSize <<= 1;
        }
    }
}
