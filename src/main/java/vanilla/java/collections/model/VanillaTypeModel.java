...
public class VanillaTypeModel<T> implements TypeModel<T> {
    ...
    private final Map<Method, MethodModel> methodMap = new LinkedHashMap<Method, MethodModel>();
    private final FieldModel[] fields;

    public VanillaTypeModel(Class<T> type) {
        this.type = type;
        Map<String, FieldModel> fieldMap = new LinkedHashMap();
        for (Method method : type.getMethods()) {
            if (method.getDeclaringClass() == Object.class) continue;

            methodMap.put(method, new VanillaMethodModel(method, fieldMap));
        }
